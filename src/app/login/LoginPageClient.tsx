  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInError) {
        ...
        console.log("LoginClient: User signed in, attempting session refresh via API route.");
        // Explicitly get session after sign-in to ensure cookies are set
        const { data: { session: newSession }, error: sessionError } = await supabase.auth.getSession();

        if (sessionError) {
          console.error("LoginClient: Error getting session after login:", sessionError);
          setError(sessionError.message);
          return;
        }

        if (newSession) {
          console.log("LoginClient: Session obtained on client, calling server refresh API.");
          // Call the new API route to ensure server-side cookies are set
          const refreshResponse = await fetch('/api/auth/refresh-session');
          const refreshData = await refreshResponse.json();

          if (refreshData.success) {
            console.log("LoginClient: Server session refreshed successfully.");
            window.location.href = '/dashboard'; // Redirect after session is confirmed by server
          } else {
            console.error("LoginClient: Server session refresh failed:", refreshData.message);
